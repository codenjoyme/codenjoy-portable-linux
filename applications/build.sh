...
if [[ "$(docker images -q java-workspace 2> /dev/null)" == "" ]]; then
    ...
    eval_echo "docker build --target java_workspace -t java-workspace ./Dockerfile --build-arg TIMEZONE=${TIMEZONE} |& tee ./logs/java-workspace.log" ;
else
...
if [[ "$(docker images -q codenjoy-source 2> /dev/null)" == "" ]]; then
    ...
    eval_echo "docker build --target codenjoy_source -t codenjoy-source ./Dockerfile --build-arg GIT_REPO=${GIT_REPO} --build-arg MAINTAINER_NAME=${MAINTAINER_NAME} --build-arg MAINTAINER_EMAIL=${MAINTAINER_EMAIL} --build-arg REF=${REVISION} |& tee ./logs/codenjoy-source.log" ;
else
...
then
    ...
    if [[ "$(docker images -q dockerized-java-workspace 2> /dev/null)" == "" ]]; then
        ...
        eval_echo "docker build --target dockerized_java_workspace -t dockerized-java-workspace ./Dockerfile-client-runner |& tee ./logs/dockerized-java-workspace.log" ;
    else
